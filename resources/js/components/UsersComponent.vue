    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4>Total <span v-if="total">({{ total }})</span></h4>
            ...
            <div class="card-body p-0">
                <div class="table-responsive table-invoice" v-if="!loading">
                    ...
                    <div v-if="!users.length" class="text-center p-3 text-muted">
                        ...
                        <p>Parece que você não adicionou usuários ainda!</p>
                    </div>
                </div>
                <div class="text-center p-4 text-muted" v-else>
                    <h5>Carregando</h5>
                    <p>Por favor espere, os dados estão sendo carregados...</p>
                </div>
            </div>
        </div>
        <div class="text-center" v-if="users.length && total > users.length">
            <button v-bind:disabled="loading" @click="loadUsers" class="btn btn-primary"><span v-if="loading">Carregando <i class="fas fa-spinner fa-spin"></i></span><span v-else>Carregar Mais</span></button>
        </div>
    </div>
</div>
</template>
    ...
    methods: {
        ...
        deleteUser(userId, index) {
            ...
            this.$iosConfirm({
                ...
                text: 'O usuário e seus dados associados serão excluídos permanentemente. Prosseguir?'
            }).then(function() {
